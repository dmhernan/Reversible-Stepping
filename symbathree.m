...
fcons = fopen(str,'w');
nout = 1e4;
tout = linspace(0,tmax,nout);
tind = 1;
h0 = h;
t = 0;
[m,x,v,om,rad,rjup] = intestpart(0);
[Q,junk] = convertcart(m,x,v);
[rlev,hlev,steps,hsub] = init(rad,h);
%     [rlev,hlev,steps,hsub] = initv(h);
[levc] = calclev(Q,v,rlev,hlev,1);
%     [levc] = calclevv(Q,v,m,rlev,hlev(1));
[jac0] = jacobinonrot(m,Q,v,om);  
nstep = 0;
levv = zeros(size(rlev,2),1);
levu = levc;
fprintf(fcons,['%0.16g %0.16g %d %0.16g %0.16g %0.16g %0.16g\n'], ...
    t,jac0,levc,norm(Q(:,3)-Q(:,2))/rad,x(1,3),x(2,3));
t1 = cputime;
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
while(t < tmax)
    nstep = nstep+1;
%         [Q,v,levc,levu,inc] = SyMBAand(Q,v,m,levc,rlev,hlev,steps,inc);
%         [Q,v,levu] = SyMBAsmooth(Q,v,m,rlev,hlev,hsub);
        [Q,v,levc,levu] = globalstep(Q,v,m,h0,levc,1,rlev,hlev);
%     [Q,v,levv,levc,levu] = mapadstep(Q,v,m,levv,levc,rlev,hlev,hsub);
%     %is there a bug in madadstep of when to calc levcp (Q) instead of Q1
%     t = t + hlev(levu);
        t = t + h;
%     if(t > tout(tind))
        t/tmax
        tind = tind + 1;
        [jac] = jacobinonrot(m,Q,v,om);
        dj = (jac - jac0)/jac0;
        [xr,yr] = rotatecoord(Q,t,om,m);
        fprintf(fcons,['%0.16g %0.16g %d %0.16g %0.16g %0.16g\n'], ...
            t,jac,levu,norm(Q(:,3)-Q(:,2))/rad,xr,yr);
%     end
...
n = size(m,2);
rlevs = size(rlev,2);
% levcn becomes a symmetric matrix;
% just redo needed pair.  If it affects any other levelc, redo whole step,
% with that new level changed if needed
%do close encounter level first?!  Redo if necessary, then other levels
%reversible?  Works if independent close encounters. What if one particle
%has several close encounters in one step?  Or situation is changing, is
%more accurate.  Just exit, show error in this case.  Let's start simple!
% Maybe need to redo whole step to keep reversibility...  OK
p0 = calcm(v,m);
...
function [Q,v,levels] = SyMBA_m(Q,v,m,h0,levc,collect_levels,rlev,hlev)
n = size(m,2);
levels = 0;
hlevn = hlev(levc);
steps = round(h0/hlevn);
j = 2;
k = 3;
p0 = calcm(v,m);
...
function [Q,v] = SyMBAsimple(Q,v,m,hlev)

p0 = calcm(v,m);
[Q] = mapSun(Q,v,hlev/2,m);
[v] = interpair(Q,v,hlev/2,m,2,3); %combine interaction Kepler steps
[Q,v] = Keppair(Q,v,hlev,m,2,3);
[v] = interpair(Q,v,hlev/2,m,2,3);
[Q] = mapSun(Q,v,hlev/2,m);
[v] = adjustSun(Q,v,m,p0); 
...
function [Q,v,levc,levu,inc] = SyMBAand(Q,v,m,levc,rlev,hlev,steps,inc)
% This will only work for three bodies so far

levu = levc;
[Q1,v1] = SyMBA(Q,v,m,levc,rlev,hlev,steps);
[levcp] = calclev(Q1,v1,rlev,hlev,1);
% [levcp] = calclevv(Q,v,m,rlev,hlev(1));
...
elseif(levcp < levc) %step is smaller; and means always smaller
    ...
    [levcpp] = calclev(Q1,v1,rlev,hlev,1);
%     [levcpp] = calclevv(Q1,v1,m,rlev,hlev(1));
    if(levcpp ~= levcp)
...
function [Q,v,levc1,levc] = globalstep(Q,v,m,h0,levc,collect_levels,rlev,hlev)
loop = 0;
%sum of steps will still be h0
[Q1,v1,levels] = SyMBA_m(Q,v,m,h0,levc,1,rlev,hlev);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Q,v,levv,levc,levu] = mapadstep(Q,v,m,levv,levc,rlev,hlev,hsub)

levu = levc;
[Q1,v1] = SyMBAsimple(Q,v,m,hlev(levc));
[levcp] = calclevsimple(Q,rlev);
rd = 0;
if(levcp > levc)
    rd = 1;
...
function [Q,v,levc,levu,inc] = SyMBAandsimple(Q,v,m,levc,rlev,hlev,inc)
% This will only work for three bodies so far
% levu is used level
% levc is next step
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
function [Q,v,levmax] = SyMBAsmooth(Q,v,m,rlev,hlev,hsub)
n = size(m,2);
p0 = calcm(v,m);
...
for j=2:n 
    for k=j+1:n
        ...
        [Q,v,levmax] = driftop(Q,v,m,2,rlev,hlev,hsub,1,j,k); %set levmax at minimum
        [v] = interpairsmooth(Q,v,hlev(1)/2,m,rlev,1,j,k);
    end
end
[Q] = mapSun(Q,v,hlev(1)/2,m);
...
function [flg] = rmincheck(Q,v,hlev,rlev,lev,jk,kk)

q = Q(:,jk) - Q(:,kk); %relative distance, 3X3
p = v(:,jk) - v(:,kk); %in inertial, relative veocity,3X3

%with q,p, code below as same as in Kepler problem
flg = 0;
qm = norm(q);
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
function [rmin] = rminget(Q,v,hlev,lev)
jk = 2;
kk = 3;
...
function [levc] = calclev(Q,v,rlev,hlev,lev)

% [rmin] = rminget(Q,v,hlev,lev);
si = size(rlev,2);
r = sqrt(sum((Q(:,2) - Q(:,3)).^2));
% r = rmin;
% r
% rlev
% blergh
...
function [levc] = calclevsimple(Q,rlev)
...
function [levc] = calclevv(Q,v,m,rlev,hglob)
global GNEWT
...
if(ratio > rlev(1)) %if greater than first curoff, then done
    levc = 1;
    return
end
for i=1:si
...
levtot = 30;
rmax = 5*rad; %Outside this value, maximum time step
rmin = 0;
delr = rmax-rmin;
rsub = (2);
hsub = 4;
...
function [m,xout,vout,pair] = in3boddunc()
global GNEWT
a0 = .0125;
a1 = 1;
e0 = .6; 
...
xout = [-fb1*x1-fb0*x0 0 0; -fb1*x1+fa0*x0 0 0; fa1*x1 0 0];
xout = xout';
vout = [0 -fb1*v1-fb0*v0 0; 0 -fb1*v1+fa0*v0 0; 0 fa1*v1 0];
vout = vout';
n = size(m,2);
for i=1:n
    for j=1:n
...
function [m,x0,v0,om,radius,rjup] = intestpart(del)
global GNEWT
G = GNEWT;
...
% del 10^{-8} to 10^{-7} can work, while still remaining within Lagrange
% points
% %take care of massless m3 IC
xhel = 4.42;
% xhel = 1.0;
...
rsq = sum(x(:,test).^2);
vsq = sum(v(:,test).^2);
ang = x(1,test)*v(2,test) - x(2,test)*v(1,test);
r1 = sqrt(sum((x(:,test)-x(:,prim)).^2));
r2 = sqrt(sum((x(:,test)-x(:,sec)).^2));


U = - G*mprim/r1 - G*msec/r2;
...
function [Q,v,levmax] = driftop(Q,v,m,lev,rlev,hlev,hsub,levmax,jk,kk)
[flg] = rmincheck(Q,v,hlev,rlev,lev-1,jk,kk);
...
if(flg == 1)
    for i=1:hsub
        if(lev > levmax)
            ...
            [Q,v,levmax] = driftop(Q,v,m,lev+1,rlev,hlev,hsub,levmax,jk,kk);
            [v] = interpairsmooth(Q,v,hlev(lev)/2,m,rlev,lev,jk,kk);
    end
elseif(flg == 0)
